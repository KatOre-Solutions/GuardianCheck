...
async function inspectData() {
  ...
  console.log("--- Validating logs Timestamps ---");
  const logsSnapshot = await db.collection("logs").get();
  
  logsSnapshot.forEach(doc => {
    const data = doc.data();
    if (!data.timestamp) {
      console.log(`Log ${doc.id} IS MISSING timestamp!`);
    } else if (typeof data.timestamp !== 'string') {
      console.log(`Log ${doc.id} has NON-STRING timestamp: ${typeof data.timestamp}`);
    }
  });
