...
export class EmailService {
  ...
  private countRead(ctx?: SendContext) {
  ...
  async sendNotification(churchId: string, childId: string, data: NotificationData, ctx?: SendContext): Promise<{ sent: number; failed: number }> {
    if (!this.db) {
    ...
    try {
      ...
      this.countRead(ctx);
      if (!childDoc.exists) return { sent: 0, failed: 0 };
      ...
      if (parentId) this.countRead(ctx);
      this.countRead(ctx);
      ...
      const results = await Promise.all(uniqueRecipients.map(async (email) => {
        ...
        const metadata = { roomName: data.roomName, serviceName: data.serviceName || "N/A" };
        ...
        if (result.ok) {
          ...
          console.error(`Failed to send email to ${email}: [${result.errorCode}] ${result.errorMessage}`);
          await this.logEmail({
            churchId,
            recipientEmail: email,
            childId,
  ...
  async sendEmergencyAlert(churchId: string, churchName: string, ctx?: SendContext) {
    try {
